...
機能: saga の本体を短く保つ
  ...
  **`rollback.feature` の変種その1。書き方だけを変える。** 振る舞いは基本形と
  ...
  この形を `example/workflow/state/workflow.go` は state 構造体とそのメソッドに割って書き、
  `example/workflow/state/workflow_flat.go` は両半分をその場に書いている。
  どちらが読みやすいかは読者が決めることで、ここで確かめるのは**2つが同じ saga で
  ある**こと。だから最後のシナリオは、同じ注文を両方に流して Receipt を比べる。
  比べていないと、片方だけ直されて静かに食い違う。

  approve のステップは下の「アクティビティ ... が実行された」に出てこない。待つのは
  ワークフローの中で完結する処理で、アクティビティを1つもスケジュールしないから。
  取り消すものも無いので、巻き戻しにも出てこない。
