...
ではなぜライブラリがあるのか。**この十数行が黙って壊れる場面が6つある**からです。どれも
本番の異常系でしか出ません。
...
以下、6つを1つずつ見ます。どれも「やりたいこと」「素直に書くと」「そこで何が起きるか」
「このライブラリだと」の順です。最後に、**意図して手放したもの**をまとめてあります。
...
握って自分のエラーを返すときだけ `s.Clear()` を呼んでください。
